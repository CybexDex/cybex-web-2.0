      ...
      <no-ssr>
        <v-flex class="orders-area">
          <v-tabs v-model="currentTab" slider-color="cybex" dark>
            ...
            <v-tab-item>
              <ExchangeOpenOrder
                :white-flag="asset_is_custom ? 'custom' : 'white'"
            ...
            <v-tab-item>
              <ExchangeOrderHistory :white-flag="asset_is_custom ? 'custom' : 'white'" :enable-interval="currentTab == 1" :order-filter="orderFilter"/>
            </v-tab-item>
            <v-tab-item>
              <ExchangeTradeHistory :white-flag="asset_is_custom ? 'custom' : 'white'" :order-filter="orderFilter"/>
            </v-tab-item>
          </v-tabs>
...
export default {
  ...
  watch: {
    ...
    currentUser: function(newVal) {
      this.loadVersionFlag(this.currentUser).then(() => {
        if (!this.versionFlag) {
          this.$nextTick(this.$toggleVersion);
        }
      });
    }
  },
